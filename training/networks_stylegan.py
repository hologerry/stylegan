...
def G_synthesis(
    ...
    if structure == 'recursive':
        ...
        def grow(x, res, lod):
            ...
            def img():
                return upscale2d(torgb(res, y), 2**lod)
...
def D_basic(
    ...
    if structure == 'recursive':
        ...
        def grow(res, lod):
            def x():
                return fromrgb(downscale2d(images_in, 2**lod), res)
            if lod > 0:
                x = cset(x, (lod_in < lod), lambda: grow(res + 1, lod - 1))
            x = block(x(), res); y = lambda: x
            if res > 2:
                y = cset(y, (lod_in > lod), lambda: tflib.lerp(x, fromrgb(downscale2d(images_in, 2**(lod + 1)), res - 1), lod_in - lod))
            return y()
